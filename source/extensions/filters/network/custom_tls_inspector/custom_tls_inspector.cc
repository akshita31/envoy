...
Config::Config(
    ...
    : stats_{ALL_TLS_INSPECTOR_STATS(POOL_COUNTER_PREFIX(scope, "tls_inspector."),
      ...
      enable_ja3_fingerprinting_(true),
...
void Filter::onALPN(const unsigned char* data, unsigned int len) {
  ...
  // todo(akshita): probaly this whole method is not needed
  //cb_->connection().connectionInfoSetter().setRequestedApplicationProtocols(protocols);
  alpn_found_ = true;
}
...
void Filter::createJA3Hash(const SSL_CLIENT_HELLO* ssl_client_hello) {
  if (config_->enableJA3Fingerprinting()) {
    ...
    std::string md5 = "akshita"; // todo(akshita): we dont need this whole section probably
    ENVOY_LOG(trace, "tls:createJA3Hash(), hash: {}", md5);

    cb_->connection().connectionInfoSetter().setJA3Hash(md5);
  }
}
